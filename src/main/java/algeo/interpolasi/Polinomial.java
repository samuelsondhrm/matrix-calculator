...
public class Polinomial {
  public static void polinomial() {
    ...
    double determinan = CofactorDeterminant.of(A);

    System.setOut(originalOut);

    if (determinan < MatrixOps.EPS) {
      System.out.println("Tidak ada solusi polinomial, karena determinan = 0");
    } else {
      System.out.println(augmentedM);
      Matrix M = MatrixOps.rref(augmentedM);
      Gauss.finishSPL(M);

      double[] coefficients = new double[n];
      for (int i = 0; i < n; i++) {
        coefficients[i] = M.get(i, n);
      }
      ...
      System.out.print("\nMasukkan nilai x lain (atau non-integer untuk keluar): ");
      ...
      while (scanner.hasNextDouble()) {
        ...
        try {
          ...
          System.out.print("Masukkan nilai x lain (atau non-integer untuk keluar): ");
        } catch (NumberFormatException e) {
          System.out.println("Input tidak valid. Harap masukkan angka atau non-integer untuk keluar.");
